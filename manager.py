...
def decrypt(key,data):
    ...
    key=key.encode('utf-8')
    aes=pyaes.AESModeOfOperationCTR(key)
    decrypted_data=aes.decrypt(base64.b64decode(data))
    return decrypted_data.decode('utf-8')

# Database Code (you can rename your database file to something less obvious)
db=sqlite3.connect("password_vault.db") 
cursor = db.cursor()
...
if (temp):
    ...
    key = key.encode('utf-8')
    insert_key = """INSERT INTO secretKey(key)
    VALUES(?) """
...
#         if txt.get() == txt1.get():
#             hashedPassword = encrypt(txt.get().encode('utf-8'))
